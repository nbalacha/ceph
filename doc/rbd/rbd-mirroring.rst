...
* **image**: When configured in ``image`` mode, mirroring must be
  `explicitly enabled`_ on each image in the default namespace of the pool.
...
Mirroring can be enabled on different namespaces in a pool independent of the
default namespace. The pool must already have been configured for mirroring.
...
* **image**: When configured in ``image`` mode, mirroring must be
  `explicitly enabled`_ on each image in the namespace.
